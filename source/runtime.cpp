...
session& runtime::open_linked_session(const std::filesystem::path& native_program_path, const std::filesystem::path& container_path)
{
    _session = std::make_unique<session>(native_program_path, container_path);
    return *(_session.get());
}
